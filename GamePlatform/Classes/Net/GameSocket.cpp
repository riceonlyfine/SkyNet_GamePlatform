...
//  GamePlatform
//
//  Created by ChenGuanhui on 1/14/16.
//
//
...
bool GameSocket::Create(const char* pszServerIP, int nServerPort, int nBlockSec, bool bKeepAlive /*= FALSE*/)
{
    ...
    in_addr_t serveraddr = inet_addr(pszServerIP);
...
bool GameSocket::recvFromSock(void)
{
    ...
    ssize_t inlen = recv(m_sockClient, m_bufInput + savepos, savelen, 0);
    if(inlen > 0) {
        // 有接收到数据
        m_nInbufLen += inlen;
        ...
        if(inlen == savelen && m_nInbufLen < INBUFSIZE) {
            ssize_t savelen = INBUFSIZE - m_nInbufLen;
